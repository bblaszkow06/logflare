...
Logflare, with the standalone DuckDB CLI; Logflare is involved only as the place to look up the
backend's config (table bucket ARN, namespace, credentials) and — after a purge — to recycle the
live query session (see [§3](#3-refresh-logflares-live-query-session)). The in-app query path
(`S3TablesAdaptor.execute_query/3`) serves endpoint reads; it is not a route for `COPY` or
`DELETE`.
...
`timestamp` and clustered by `(project, source_uuid, timestamp)` (O11Y-2215). Both tenancy
columns are **required** (NOT NULL): the mapper always emits them, writing `""` for `project`
when the event body has no project path. Because `project` leads the sort key, put a `project`
predicate alongside `source_uuid` whenever you know it — a `source_uuid` filter on its own
prunes far less. Any per-source operation must cover all three tables. Dotted column names
(`"events.timestamp"`, …) are flat columns and must be double-quoted in DuckDB.
...
- **Data coordinates**: the source UUID(s) (the source `token` in Logflare) and the `project`
  value the mapping config writes (`""` for sources whose events carry no project path).
...
> **Performance warning**: batches are sorted to the table's sort order at append time
> (O11Y-2215), but files written *before* that landed are not clustered, so a
> `project`/`source_uuid` filter cannot prune them — the query scans **all data of the backend**
> for the covered days. Pre-existing files are only re-clustered by compaction, and only if the
> table's maintenance config sets `compaction.strategy = sort`; under the default `binpack`
> strategy compaction may interleave rows again (see the `IcebergSchema` moduledoc). This
> affects cost and duration only, not correctness. The `timestamp` bounds do prune day
...
WHERE project = '<project>' AND source_uuid = '<source_uuid>'
  AND "timestamp" >= TIMESTAMPTZ '2026-07-01 00:00:00+00'
  AND "timestamp" <  TIMESTAMPTZ '2026-08-01 00:00:00+00';
...
Hive-style output layout. Note `project` is required but mapper-populated: its value is whatever
the source's mapping config produces, and `""` for events with no project path. If unsure what a
source writes there, sample it first (`SELECT project, count(*) … GROUP BY 1`) rather than
guessing.
...
2. The external DELETE commit contends with the pipeline's `fast_append` commits. The pipeline
   retries a failed batch once (`Pipeline.max_retries/0`, on top of iceberg-rust's internal
   30 s commit retry from `commit.retry.total-timeout-ms`) and then **drops the events with only
   a log warning** (`"Dropping N S3 Tables events"`).
...
at table-creation time (see [Eventual Logflare changes](#4-eventual-logflare-changes)) so the
...
## 3. Refresh Logflare's live query session

A purge changes what a *new* DuckDB session sees, but Logflare keeps a long-lived one per
backend: `QuerySession` runs its bootstrap (`INSTALL`/`LOAD`, `CREATE SECRET`, `ATTACH`, `USE`)
once on the first query and reuses that connection for every query afterwards. A session that
was already attached before the DELETE may therefore keep answering endpoint reads from a
pre-purge snapshot.

Recycle the backend's query subtree so the next query re-bootstraps and re-`ATTACH`es:
...
`QueryBackendSup` supervises `Adbc.Database` → `Adbc.Connection` → `QuerySession` as
`:rest_for_one` and is `:temporary` under `QuerySup`, so terminating it drops every piece of
DuckDB state for that backend; `QuerySup.ensure_session/1` starts a fresh subtree on the next
query. `QuerySup` is node-local — run this on every node that has served a query for the
backend (cluster fan-out is listed below as future work).
...
1. **Pin maintenance config at table creation** (`CatalogManager`): call
   `PutTableMaintenanceConfiguration` after `ensure_table`, so the purge SLA, storage costs and
   `compaction.strategy = sort` — which is what keeps append-time-sorted files clustered — stop
   depending on unpinned AWS account defaults. Keep the invariant of never setting
...
2. **`Maintenance.purge/3` / `Maintenance.export/4` module**: own DuckDB connection, bypassing —
   not weakening — the read-only endpoint query path. Requires bumping `adbc`/DuckDB so the
   embedded engine gains partitioned DELETE; doing it in the NIF instead would depend on
   iceberg-rust delete-write support (crates pinned at 0.10.1).
...
4. **Telemetry/alarm on conflict-drops** in `Pipeline.ack_backend_failures/2`, so purge-induced
   (or compaction-induced) ingest loss is observable.
5. **Per-backend ingest-pause flag** consulted by the pipeline, for race-free purges without
   touching senders.
